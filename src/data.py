...
def load_binary_classification_data(
    ...
    """Load a digits/MNIST split and map configured class groups to -1/+1."""
    ...
    y = data["y_train"]
    neg_mask = torch.zeros_like(y, dtype=torch.bool)
    pos_mask = torch.zeros_like(y, dtype=torch.bool)
    ...
    y_binary = torch.where(pos_mask[keep], 1.0, -1.0).to(device=X_train.device, dtype=X_train.dtype)
