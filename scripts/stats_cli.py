...
def configure_repo_root(script_path: str) -> Path:
    """Require repo-root execution and make repository imports available."""
    ...
    if Path.cwd().resolve() != repo_root:
        raise SystemExit(
            ...
            f"  python {Path(script_path).relative_to(repo_root)} --config <path>"
        )
...
def compact_config(value: Any, field_name: Optional[str] = None) -> Any:
...
def dump_compact_json(value: Any, indent: int = 2, level: int = 0) -> str:
...
def run_experiment_cli(
    *,
    experiment_name: str,
    config_cls: Type[Any],
    run_experiment: Callable[[Any], tuple],
    plot_from_rows: Callable[..., tuple],
    default_rows_filename: str,
) -> None:
    """Shared command-line runner for stats experiments."""
    args = _parse_args(experiment_name, default_rows_filename)
    print(datetime.now())
    ...
    print(f"plots: {len([key for key in paths if key.startswith('plot_')])}")
    print(datetime.now())


def _parse_args(experiment_name: str, default_rows_filename: str) -> argparse.Namespace:
    p = argparse.ArgumentParser(description=f"Run the {experiment_name} experiment.")
    p.add_argument("--config", type=Path, required=True, help="Path to YAML/JSON experiment config.")
    p.add_argument("--output-dir", type=Path, default=None, help="Override experiment.output_dir from the config.")
    p.add_argument("--device", type=str, default=None, help="Override experiment.device, e.g. cpu, cuda, cuda:0, auto.")
    p.add_argument(
        "--plot-only",
        action="store_true",
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help=f"Raw rows CSV for --plot-only. Defaults to experiment.output_dir/{default_rows_filename}.",
    )
    return p.parse_args()


def _load_config(config_cls: Type[Any], path: Path) -> Any:
    mapping = _load_mapping(path)
    if "probe" in mapping:
        raise ValueError("Config key 'probe' was renamed to 'experiment'. Please update the YAML/JSON file.")
    return config_cls(**dict(mapping.get("experiment", mapping)))
...
def _compact_list(values: list, field_name: Optional[str] = None) -> Any:
...
def _compact_name_list(values: list) -> dict:
...
def _summarize_scalar_list(values: list) -> dict:
...
def _constant_step(values: list) -> Optional[Number]:
